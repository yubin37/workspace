...
/* Pre-device (per-bank) structure */
struct cmos_dev {
	unsigned short current_point; /* current pointer within bank */
...
struct class *cmos_class;             /* Tie with device model */
...
{
	struct cmos_dev *cmos_devp = fil->private_data;
...
int __init cmos_init(void)
{
	...
	for (i = 0; i < NUM_CMOS_BANKS; i++) {
		...
		if (!(request_region(addrports[i], 2, cmos_devp[i]->name))) {
			printk("cmos: I/O port 0x%x is not freel.\n", addrports[i]);
			return -EIO;
		}
